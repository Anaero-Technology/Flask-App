    ...
    def connect(self, port: str, device_name: Optional[str] = None) -> bool:
        ...
        with self._app.app_context():
            ...
            handlers_to_try = [
                ...
                ('black-box', BlackBoxHandler)
            ]
            
            for device_type, HandlerClass in handlers_to_try:
                handler = None
                try:
                    handler = HandlerClass(port)
                    ...
                    response = handler.send_command("info", timeout=3.0)
                    if response and ("info" in response or "chimera" in response or "black" in response):
                        # Connection successful
                        if not device:
                            name_to_use = device_name or ("Chimera Device" if device_type == "chimera" else "BlackBox Device")
                            device = Device(
                                name=name_to_use,
                                device_type=device_type,
                                serial_port=port,
                                mac_address=getattr(handler, 'mac_address', None),
                                connected=True,
                                logging=getattr(handler, 'is_logging', False)
                            )
                        ...
                        print(f"Successfully connected to {device_type} on {port}")
                        return True
                        
                except Exception as e:
                    print(f"Failed to connect as {device_type} on {port}: {e}")
                    if handler:
                        try:
                            handler.disconnect()
                        except:
            ...
            print(f"Could not connect to any device type on {port}")
            return False
