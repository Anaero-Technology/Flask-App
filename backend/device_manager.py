...
class DeviceManager:
    ...
    def connect(self, port: str, device_name: Optional[str] = None) -> Dict:
        """
        ...
        if port in self._active_handlers:
            # Find the device in DB with this port
        ...
        try:
            ...
            if temp_handler:
                temp_handler.disconnect()
            return {"success": False, "error": f"Failed to detect device type: {str(e)}"}
