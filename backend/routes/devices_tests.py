from flask import Blueprint, request, jsonify, send_file, current_app, has_app_context
...
def get_device_manager():
    # Request handlers run with app context; worker threads may not.
    if has_app_context():
        return current_app.extensions['device_manager']
    return DeviceManager()


@devices_tests_bp.route('/api/v1/ports')
@jwt_required()
def list_ports():
    ports = []
...
@devices_tests_bp.route("/api/v1/devices")
@jwt_required()
def list_devices():
    try:
...
@devices_tests_bp.route("/api/v1/devices/<int:device_id>", methods=['GET'])
@jwt_required()
...
@devices_tests_bp.route("/api/v1/devices/<int:device_id>", methods=['PUT'])
@jwt_required()
...
def update_device(device_id):
    ...
    try:
        ...
        # Get handler
        handler = get_device_manager().get_black_box(device_id)
        if not handler:
...
@devices_tests_bp.route("/api/v1/devices/<int:device_id>", methods=['DELETE'])
@jwt_required()
@require_role(['admin', 'operator'])
...
@devices_tests_bp.route("/api/v1/devices/by_mac/<mac_address>")
@jwt_required()
...
@devices_tests_bp.route("/api/v1/devices/discover")
@jwt_required()
@require_role(['admin', 'operator'])
def discover_devices():
    ...
    valid_devices = []
    lock = threading.Lock()
    # Resolve manager once in request context; worker threads must not touch current_app.
    ...
    def check_port(port_info):
        ...
        try:
            ...
            connected = manager.connect(port_info.device)
            
            if not connected:
                return
            
            device = manager.get_device_by_port(port_info.device)
...
@devices_tests_bp.route("/api/v1/devices/connect", methods=['POST'])
@jwt_required()
@require_role(['admin', 'operator'])
def connect_device():
    ...
    try:
        ...
        result = get_device_manager().connect(serial_port, device_name)
...
def disconnect_device_by_port(port):
    ...
    try:
        success = get_device_manager().disconnect_by_port(port)
...
def disconnect_device(device_id):
    ...
    try:
        success = get_device_manager().disconnect_device(device_id)
...
@devices_tests_bp.route("/api/v1/samples", methods=['GET'])
@jwt_required()
...
@devices_tests_bp.route("/api/v1/samples/<int:sample_id>", methods=['DELETE'])
@jwt_required()
@require_role(['admin', 'operator'])
...
@devices_tests_bp.route("/api/v1/tests", methods=['POST'])
@jwt_required()
@require_role(['admin', 'operator'])
def create_test():
    """Create a new test"""
...
@devices_tests_bp.route("/api/v1/tests", methods=['GET'])
@jwt_required()
def list_tests():
    """Get all tests"""
    try:
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>", methods=['GET'])
@jwt_required()
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>", methods=['PUT'])
@jwt_required()
@require_role(['admin', 'operator'])
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>/chimera-configuration", methods=['GET'])
@jwt_required()
...
def start_test(test_id):
    ...
    try:
        ...
        for chimera_device in chimera_devices:
            ...
            chimera_handler = get_device_manager().get_device(chimera_device.id)
        ...
        for device_id in all_device_ids:
            device = Device.query.get(device_id)
            handler = get_device_manager().get_device(device_id)
...
def stop_test(test_id):
    ...
    try:
        ...
        stop_results = []
        for device in devices:
            handler = get_device_manager().get_device(device.id)
            result = None
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>/configurations", methods=['POST'])
@jwt_required()
@require_role(['admin', 'operator'])
        ...
        if affected_device_ids:
            ...
            with BlackBoxHandler._db_write_lock:
                ...
                for device_id in blackbox_device_ids:
                    ...
                    handler.app = current_app._get_current_object()
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>/chimera-configuration", methods=['POST'])
@jwt_required()
@require_role(['admin', 'operator'])
...
@devices_tests_bp.route("/api/v1/tests/upload-csv", methods=['POST'])
@jwt_required()
@require_role(['admin', 'operator'])
...
@devices_tests_bp.route("/api/v1/tests/<int:test_id>", methods=['DELETE'])
@jwt_required()
@require_role(['admin', 'operator'])
