...
def discover_devices():
    ...
    def check_port(port_device):
        """Check a single port for valid device"""
        # Skip Bluetooth ports to avoid blocking issues
        if isinstance(port_device, str) and 'Bluetooth' in port_device:
            return
        
        port_name = port_device if isinstance(port_device, str) else port_device.device
        
        try:
            # Connect to the device
            connected = device_manager.connect(port_name)
            
            if not connected:
                return
            
            device = device_manager.get_device_by_port(port_name)
            if device and device.device_type in ['black-box', 'chimera']:
                with lock:
            ...
            print(f"Error checking port {port_name}: {str(e)}")
            pass
    
    # Get all available serial ports
    ports = list(serial.tools.list_ports.comports())

    # Check if ttyS0 is already in the list to avoid duplication
    ttyS0_exists = any(port.device == '/dev/ttyS0' for port in ports)

    # Add Raspberry Pi UART port only if not already detected
    if not ttyS0_exists:
        uart_port = type('MockPort', (), {'device': '/dev/ttyS0', 'name': 'ttyS0', 'description': 'Raspberry Pi UART'})()
        ports.append(uart_port)
