...
function TestForm() {
    ...
    const TEST_FORM_STORAGE_KEY = 'test_form_state_v1';
    ...
    useEffect(() => {
        try {
            const savedState = sessionStorage.getItem(TEST_FORM_STORAGE_KEY);
            if (!savedState) return;
            const parsed = JSON.parse(savedState);

            if (typeof parsed.testName === 'string') setTestName(parsed.testName);
            if (typeof parsed.testDescription === 'string') setTestDescription(parsed.testDescription);
            if (Array.isArray(parsed.selectedDevices)) setSelectedDevices(parsed.selectedDevices);
            if (parsed.configurations && typeof parsed.configurations === 'object') setConfigurations(parsed.configurations);
            ...
            console.error('Failed to restore test form state:', error);
        } finally {
            setDidRestore(true);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        try {
            ...
            sessionStorage.setItem(TEST_FORM_STORAGE_KEY, JSON.stringify(payload));
        } catch (error) {
            console.error('Failed to persist test form state:', error);
        }
    }, [
        didRestore,
        testName,
    ...
    const fetchDevices = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const availableDeviceIds = data.map(d => d.id);
                setSelectedDevices(prev => {
                    if (!prev.length) return prev;
                    return prev.filter(id => availableDeviceIds.includes(id));
                });
            }
        } catch (error) {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col space-y-6">
                ...
                {/* Chimera Configuration - Show if any Chimera device is selected */}
                {hasChimeraSelected() && (
                    <ChimeraTestConfig
                        flushTime={flushTime}
