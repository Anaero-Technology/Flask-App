...
- Machine personalities (`systemset <x>`): `lobster ray max caterpillar blackswan medusa`. Nothing but a few identity/config commands work until a personality is set (`nosystem` otherwise).
...
| `frontend/src/view/plcLayouts.js` | **Machine models** mapped over shared firmware personalities: `ray`+`ray-i`→firmware `ray`, `lobster-i`→firmware `max`, etc. Each model has `feeders:[[n,[reactors]]]` and optional `downstream:{}`. `resolveModel`, `feederForReactor`. |
...
- **Ray/Ray-I & Lobster-I share a firmware personality** — the PLC reports the same unit counts for both; the model is a UI-only choice stored per-MAC. `ray` firmware reports 2 feeders even though a plain Ray has one wired — layouts filter to the model's feeders.
